...
**Analytics & AI > AI Data Platform Workbenches > Create AI Data Platform Workbench** is a single page. It asks for a Workbench name and a default workspace name in one step (no separate "create workspace" screen), a Lakehouse configuration — **Choose existing**, **Create new**, or **None** — and a storage-encryption choice. Picking **Create new** for the Lakehouse only asks for two fields: a fixed `ADMIN` username and a password. There's no ECPU or storage sizing exposed at all; AIDP owns those defaults and provisions the underlying Autonomous AI Lakehouse itself.
...
Click **Add**, then **Create**, and once the instance goes Active you land on the Workbench home page:
...
With the Workbench up, **Master Catalog** needs to be created first. **Master Catalog > Create Catalog** asks for a catalog name, a type (pick Standard catalog), and an optional compartment override; leave it matching the Workbench's own compartment unless you specifically want the catalog's storage elsewhere:
...
After that, we need to wait until it shows up Active alongside the platform's own `default` catalog:
...
**Custom** exposes everything: driver/worker shape, OCPUs, memory, block volume size, and whether the worker count is static or autoscaled. For this demo, with a raw TfL poll landing around 15k rows every 10 seconds (well under that after the producer's own dedupe), a single static worker at the smallest available shape is plenty (we'll see about that, but these settings can be changed later, so let's go small first):
...
Hit Create, and, notice that, *two* clusters start Creating, not one: `tfl_cluster`, and a second, system-managed **Default Master Catalog Compute** cluster that came along for the ride from the catalog step above. 

This *creating* step can take some time, so just wait until it's done.
...
OCI Vault keys default to **HSM** protection mode (hardware-backed). Once created, the key list confirms it as Enabled, HSM, AES:
...
The **Create secret** form defaults to **Automatic secret generation** (a randomly generated passphrase), which is the wrong choice here.  We will use `KAFKA_USERNAME` and `KAFKA_PASSWORD`, which are specific, known-shape values (a compound OCI Streaming username, and an OCI Auth Token). Switch to **Manual secret generation** exposes a plain text box instead:
...
Both secrets exist as placeholders at this point. We did it deliberately ad will get back to this setting later in series when we define OCI Streaming. OCI Vault versions secrets, so swapping in real values later is a new secret *version*, not a delete-and-recreate:
